...
public class AddMovieWindow {
	...
	public boolean NewAddMovieWindow(Connection c, settingsmanager settings){
		this.settings = settings;
		Stage window = new Stage();
		window.initModality(Modality.APPLICATION_MODAL);
		window.setTitle("Film hinzufügen");
		window.setWidth(800);
		window.setHeight(400);
		window.setResizable(false);
		int nextid = DBmanager.getNextID();
		...
		mainframe.setStyle("-fx-background-color: white");
			...
			HBox titlebox = new HBox();
				...
				Label nextidlbl = new Label("Nächste freie ID: "+String.valueOf(nextid));
			...
			HBox confirmcancelframe = new HBox();
				...
				confirm.setStyle("-fx-background-color: white; -fx-font-size: 20pt");
				...
				confirm.setOnAction(new EventHandler<ActionEvent>() {
					...
					public void handle(ActionEvent event) {
						...
						for(Movie m:DBmanager.getAllMovies()){
							if(m.getTitle().toLowerCase().trim().equals(title.toLowerCase().trim())) available = true;
						}
						//title verification
						if(title != null && !available && !title.trim().isEmpty() && length < 5000){
							...
							Movie m = new Movie(id, title, ifsk, length, loc, series, category, dsc);
							DBmanager.addMovie(m);
							window.close();
				...
				cancel.setStyle("-fx-background-color: white; -fx-font-size: 20pt");
